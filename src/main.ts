...
export default class JourneyPlugin extends Plugin {
	...
	async onload() {
		...
		this.addRibbonIcon('journey', 'Map journey', () => {
			this.startSearch();
		});
	}
	...
	public async findShortestPath(start: string, end: string) {
		...
		let noteInitContent = "some other text"
		console.log(finalList)
		var finalString = "# Journey between " + (finalList[0].split("/")[finalList[0].split("/").length -1]).split(".m")[0] + " and " + (finalList[finalList.length-1].split("/")[finalList[finalList.length-1].split("/").length -1]).split(".m")[0];
		console.log(finalList.length)
		for (var i = 0; i < finalList.length; i++) {
			finalString = finalString + "\n" + "![[" + finalList[i] + "]]\n" + "\n" + "---" + "\n"
			console.log(finalList[i])
		};
		let pathString = (finalList[0].split("/")[finalList[0].split("/").length -1]).split(".m")[0] + "-" + (finalList[finalList.length-1].split("/")[finalList[finalList.length-1].split("/").length -1]).split(".m")[0]
    await this.app.vault.adapter.write('Journeys/'+pathString+'.md',finalString);
